  ...
  main.variable(observer()).define(["makeTrees","d3","trees"], function*(makeTrees,d3,trees)
{
  ...
  const text = svg.append("text").attr("x", 0).attr("y", 1);
  ...
  function brushCalled(event) {
    ...
    text.text('Trees Selected: ' + dataPoints.length).style("font-size", "0.8px");
  }
  svg.append("g")
    .attr("class", "mybrush")
  ...
  main.variable(observer()).define(["makeSigns","d3","signs"], function*(makeSigns,d3,signs)
{
  ...
  function brushCalled(event) {
    ...
    .filter(".selected")
    .each(d => dataPoints.push(d));
    text.text('Signs Selected: ' + dataPoints.length).style("font-size", "0.8px");
  }
  svg.append("g")
    .attr("class", "mybrush")
    .style("stroke-width", 0.01)
